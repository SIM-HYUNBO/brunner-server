	...
	public void init() {
		...
		RPCServer server = new RPCServer() {
			...
			protected JsonObject executeService(JsonObject request) {
				String txnId = request.get(Constants.msgFieldName_inputData).getAsJsonObject()
						.get(Constants.msgFieldName_txnId).getAsString();
		...
		try {
			server.start(RABBITMQ_HOST, RABBITMQ_PORT, RABBITMQ_USERNAME, RABBITMQ_PASSWORD, queueName);
		} catch (IOException e) {
	...
	JsonObject executeBrunnerService(JsonObject request) {
		...
		try {
			if (request.get(Constants.msgFieldName_inputData).getAsJsonObject().has(Constants.msgFieldName_txnId))
				txnId = request.get(Constants.msgFieldName_inputData).getAsJsonObject()
						.get(Constants.msgFieldName_txnId).getAsString();
			else {
				txnId = StringUtil.getTxnId();
				request.get(Constants.msgFieldName_inputData).getAsJsonObject()
						.addProperty(Constants.msgFieldName_txnId, txnId);
			}
		...
		int timeoutS = request.has("requestTimeoutMS") && request.get("requestTimeoutMS").getAsInt() > 0
				? request.get("requestTimeoutMS").getAsInt() / 1000
				: 60;
